...
text(x=c(2,6,10,14)+0.3, y=(pit_mean_diff$ParasiteMean + pit_mean_diff$HostMean) * 500,
...
# Plot 
wave <-plot_ly(x = ~pit_membrane_thickness,
        y = ~number_of_pits,
        z = ~background_plot) %>%
  layout(
  ...
  add_trace(x = rep(mean_ptm_plant1, length(number_of_pits)), 
            y = number_of_pits, 
