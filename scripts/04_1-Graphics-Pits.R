...
boxplot(
  data = pitdata_clean, pitavg * 1000 ~ ssp, na.rm = T, las = 2, cex = 1,
...
text(
  x = c(1, 3, 5, 7, 9, 11, 13, 15), y = -150, c("P. robustus", "V. thyrsoidea", "P. perrottetti", "T. guianensis", "S. rhynchophyllus", "T. tipu", "V. album", "P. nigra"),
...
segments(x0 = c(2, 6, 10, 14), y0 = pit_mean_diff$ParasiteMean * 1000, y1 = pit_mean_diff$HostMean * 1000, lwd = 2)
text(
  x = c(2, 6, 10, 14) + 0.3, y = (pit_mean_diff$ParasiteMean + pit_mean_diff$HostMean) * 500,
  round(pit_mean_diff$MeanDifference, 3) * 1000, cex = 1.2
)


boxplot(
  data = pitdata_clean, pcd * 1000 ~ ssp, na.rm = T, las = 2, cex = 1,
  ylab = "", xlab = NA, tcl = T, xaxt = "n",
  col = c("firebrick", "grey"), at = c(1, 3, 5, 7, 9, 11, 13, 15),
...
title(ylab = "Pit Chamber depth (nm)", line = 4, cex.lab = 1.5)
text(
  x = c(1, 3, 5, 7, 9, 11, 13, 15), y = -300, c("P. robustus", "V. thyrsoidea", "P. perrottetti", "T. guianensis", "S. rhynchophyllus", "T. tipu", "V. album", "P. nigra"),
  xpd = NA, cex = 1.5, srt = 35, col = "black", adj = 0.50, font = 3
)
axis(1, at = c(1, 3, 5, 7, 9, 11, 13, 15), labels = NA)
