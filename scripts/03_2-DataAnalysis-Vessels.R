...
levels(Hydraulic_means$parasitism)
...
sum(abs(boot_diffs[,1])>=abs(boot_diffs[1,1]))
...
for (pair in species_pairs) {
  ...
  for (v in vars) {
    ...
    resample_data <- as.data.frame(t(replicate(5, shuffle_means(subset_data, cols = v, cat = "ssp", rcol = TRUE))))
...
ssp_CI95 <- t(sapply(pair_boot,simplify = T,function (y)
apply(y[,vars],2,function(x){
  quantile(x,c(0.025,0.975))
})))
colnames(ssp_CI95) <- colnames(ssp_CI95) <- unlist(mapply(function(var) {
  c(paste(var, "lower", sep = "_"), paste(var, "upper", sep = "_"))
}, vars, SIMPLIFY = FALSE))
...
# Print the resulting list with data frames
print(ssp_diffs_list)
...
ssp_pvalues<- sapply(ssp_diffs_list,simplify = T,function(x){
