...
boot2 <- matrix(nrow = Iterations, ncol = 2)
colnames(boot2) <- c("pcd", "Tpm")  # Definindo os nomes das colunas para o boot2

for (v in c("pcd", "Tpm")) {
  boot2[, v] <- replicate(Iterations, mean_diff_boot(TEM_data, cols = v, cat = "parasitism"))
}

Bootstraped_data$Means_parasitism <- cbind(Bootstraped_data$Means_parasitism,boot2)


boot3 <- matrix(nrow = Iterations, ncol = 2)
colnames(boot3) <- c("pcd", "Tpm")  # Definindo os nomes das colunas para o boot2

for (v in c("pcd", "Tpm")) {
  boot3[, v] <- replicate(Iterations, median_diff_boot(TEM_data, cols = v, cat = "parasitism"))
}

Bootstraped_data$Medians_parasitism <- cbind(Bootstraped_data$Medians_parasitism,boot3)
...
for (pair in species_pairs) {
  # Subset data for the species pair
  subset_data <- subset(TEM_data, ssp %in% pair)
  pair_id <- paste(pair, collapse = "X")
  print(pair_id)
  
  # Initialize a matrix for bootstrap values
  m <- matrix(NA, nrow = Iterations, ncol = 2)
  colnames(m) <- c("pcd", "Tpm")  # Set column names for the matrix
  ...
  # Create the table name based on the species pair
  table_name <- paste("Means_", pair_id, sep = "")
  
  # Check if the table name exists in the list and append the result
  if (table_name %in% names(Bootstraped_data)) {
    # Append the matrix to the corresponding table in the Bootstraped_data list
    Bootstraped_data[[table_name]] <- cbind(Bootstraped_data[[table_name]], m)
  } else {
    # If the table doesn't exist, create a new entry in the list
   print(error)
  }
}
...
for (pair in species_pairs) {
  ...
  for (v in colnames(m)) {
    # Generate bootstrap values for the current variable
    boot <- replicate(Iterations, median_diff_boot(subset_data, cols = v, cat = "ssp"))
  ...
  # Create the table name based on the species pair
  table_name <- paste("Medians_", pair_id, sep = "")
...
for (p in levels(TEM_data$parasitism)) {
  subset_data <- subset(TEM_data, parasitism == p)
  para_type <- paste(p, "CI95", sep = "_")
  
  # Generate bootstrap values for all variables in one matrix
  m <- sapply(Variables, function(v) {
    replicate(Iterations, mean(sample(subset_data[[v]], replace = TRUE), na.rm = TRUE))
  })
  
  table_name <- paste("Means_", para_type, sep = "")
  
  # Check if the table name exists in the list and append the result
  if (table_name %in% names(CI95)) {
    # Append the matrix to the corresponding table in the Bootstraped_data list
    CI95[[table_name]] <- cbind(CI95[[table_name]], m)
  } else {cat(":(")
  }
}
...
for (p in levels(TEM_data$parasitism)) {
  ...
  # Generate bootstrap values for all variables in one matrix
  m <- sapply(Variables, function(v) {
    replicate(Iterations, median(sample(subset_data[[v]], replace = TRUE), na.rm = TRUE))
  })
  
  table_name <- paste("Medians_", para_type, sep = "")
  ...
  for (p in levels(TEM_data$ssp)) {
    subset_data <- subset(TEM_data, ssp == p)
    para_type <- paste(p, "CI95", sep = "_")
...
for (p in levels(TEM_data$ssp)) {
  ...
  # Generate bootstrap values for all variables in one matrix
  m <- sapply(Variables, function(v) {
    replicate(Iterations, median(sample(subset_data[[v]], replace = TRUE),na.rm=T))
  })
  table_name <- paste("Medians_", para_type, sep = "")
