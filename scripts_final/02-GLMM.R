...
PitDiOp_data[616,"PitDiameter"] <-
  rev(sort(PitDiOp_data$PitDiameter[PitDiOp_data$indiv=="Tapirira guianensis 1"]))[2]
PitDiOp_data[354,"PitDiameter"]  <- 
  rev(sort(PitDiOp_data$PitDiameter[PitDiOp_data$indiv=="Psittacanthus robustus 2"]))[2]
  PitDiOp_data[531,"PitDiameter"] <- 
  rev(sort(PitDiOp_data$PitDiameter[PitDiOp_data$indiv=="Struthanthus rhynchophyllus 2"]))[2]
# Fit global models
...
CookD(PitDiameter_full,newwd = F)
...
PitDiOp_data[607,"PitOpening"] <- 
  rev(sort(PitDiOp_data$PitOpening[PitDiOp_data$indiv=="Tapirira guianensis 1"]))[2]
  PitDiOp_data[1083,"PitOpening"] <- 
    sort(PitDiOp_data$PitOpening[PitDiOp_data$indiv=="Vochysia thyrsoidea 1"])[2]
# Fit global models
PitOpening_full <- lme(
...
PitDiOp_data[615,"PitOpening"] <- 
  rev(sort(PitDiOp_data$PitOpening[PitDiOp_data$indiv=="Tapirira guianensis 1"]))[2]
PitDiOp_data[c(797,792),"PitOpening"] <- 
  rev(sort(PitDiOp_data$PitOpening[PitDiOp_data$indiv=="Tipuana tipu 1"]))[3]
# Iterate through species pairs
for (pair in species_pairs) {
  subset_data <- subset(PitDiOp_data, ssp %in% pair) %>% na.omit()
